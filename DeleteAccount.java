    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
         ...
         List<Integer> ids_list;
             ...
             ids_list = PhotosDB.getPhotoIDs(200, userName);
             for(int i=0 ; i<ids_list.size() ; i++)
                PhotosDB.deletePhoto(ids_list.get(i));
             
             List<Comment> comments_list = CommentDB.getComments();
              for(int i=0 ; i<comments_list.size() ; i++)
                  if(comments_list.get(i).getUserName().equals(userName))
                      CommentDB.deleteComment(comments_list.get(i));
                     
               List<Rating> ratings_list = RatingDB.getRatings();
              for(int i=0 ; i<ratings_list.size() ; i++)
                  if(ratings_list.get(i).getUserName().equals(userName))
                      RatingDB.deleteRating(ratings_list.get(i));
